...
class verify {
    ...
    public function check($code, $id = '') {
        ...
        // 验证码不能为空
        $secode =  Session::get($key); //Session($key);
        if(empty($code) || empty($secode)) {
            return false;
        }
        // session 过期
        if(NOW_TIME - $secode['verify_time'] > $this->expire) {
            Session::set($key,null);//session($key, null);
            return false;
        }

        if($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            $this->reset && Session::set($key,null);//session($key, null);
            return true;
        }
    ...
    public function entry($id = '') {
        ...
        Session::set($key.$id, $secode);//session($key.$id, $secode);
