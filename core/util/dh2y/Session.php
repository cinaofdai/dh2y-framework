...
class Session {
    ...
    static function start() {
        ...
        Session::setExpire(\core\lib\config::get('SESSION_EXPIRE','config'));
    }
    ...
    static function _init() {
        ...
        // 设置Session有效域名
        Session::setCookieDomain(\core\lib\config::get('COOKIE_DOMAIN','config'));
        //设置当前项目运行脚本作为Session本地名
        Session::localName(DH2Y);
        Session::name(\core\lib\config::get('SESSION_NAME','config'));
        Session::path(\core\lib\config::get('SESSION_PATH','config'));
        Session::setCallback(\core\lib\config::get('SESSION_CALLBACK','config'));
    }
