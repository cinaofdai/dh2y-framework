...
class upload {
    ...
    private function setDriver($driver = null, $config = null){
        $driver = $driver ? : ($this->driver       ? : config::get('FILE_UPLOAD_TYPE','config'));
        $config = $config ? : ($this->driverConfig ? : config::get('UPLOAD_TYPE_CONFIG','config'));
        $class = strpos($driver,'\\')? $driver : '\core\lib\drive\upload\\'.ucfirst(strtolower($driver));
