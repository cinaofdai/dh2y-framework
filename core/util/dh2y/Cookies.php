...
class Cookies{ // class start
    ...
    private $_securekey;                                              // encrypt key
    private $_expire;                                                // default expire
    ...
    public function __construct($prefix='', $expire=0, $securekey=''){
        ...
        if(is_string($prefix) && $prefix!=''){
            ...
            $this->_prefix =  \core\lib\config::get('COOKIE_PREFIX','config');
        }

        if(is_numeric($expire) && $expire>0){
            $this->_expire = $expire;
        }else{
            $this->_expire = \core\lib\config::get('COOKIE_EXPIRE','config');                                                // default expire
        ...
        if(is_string($securekey) && $securekey!=''){
            ...
            $this->_securekey = \core\lib\config::get('COOKIE_SECUREKEY','config');
        }
    ...
    public function update($name, $value){
        ...
        if(isset($_COOKIE[$cookie_name])){
            ...
            if(isset($old_cookie_value[1]) && $old_cookie_value[1]>0){ // 获取之前的过期时间
                ...
                if($cookie_name && $cookie_value && $cookie_expire){
                    //cookie($cookie_name, $cookie_value, $cookie_expire);
                    setcookie($cookie_name, $cookie_value, $cookie_expire);
                    return true;
                }
